(function ($) {
  ...
  lm.utils.copy(lm.LayoutManager.prototype, {
    ...
    _$reconcilePopoutWindows: function () {
      var openPopouts = [], i, window;

      for (i = 0; i < this.openPopouts.length; i++) {
        window = this.openPopouts[i] && this.openPopouts[i].getWindow();
        if (window && window.closed === false) {
    ...
    _adjustToWindowMode: function () {
      ...
      document.title = lm.utils.stripTags(this.config && this.config.content && this.config.content[0] && this.config.content[0].title || '');
    ...
    _create: function (config) {
      ...
      if (!(config.content instanceof Array)) {
        ...
        console.warn('Missing setting \'content\' on top level of configuration');
        config.content = [];
      }
  ...
  lm.utils.copy(lm.controls.BrowserPopout.prototype, {
    ...
    _createWindow: function () {
      ...
      var checkReadyInterval,
        url = this._createUrl(storageKey),
      ...
      if (!this._popoutWindow) {
        if (this._layoutManager.config.settings.blockedPopoutsThrowError === true) {
          alert('Please allow open pop-out in your browser!');
          return;
    ...
    _positionWindow: function () {
      this._dimensions.left = 0;
      this._dimensions.top = 0;
  ...
  lm.utils.copy(lm.controls.Header.prototype, {
    ...
    _updateTabSizes: function (showTabMenu) {
      ...
      for (i = 0; i < this.tabs.length; i++) {
        ...
        //Include the isActive tab's width if it isn't already
        //This is to ensure there is room to show the isActive tab
        if (activeIndex <= i) {
        ...
        if (visibleTabWidth > availableWidth) {
          ...
          if (!tabOverlapAllowanceExceeded) {
            ...
            //No overlap for first tab or isActive tab
            //Overlap spreads among non-isActive, non-first tabs
  ...
  lm.controls.Tab = function (header, contentItem) {
    ...
    if (this.contentItem.config.isClosable) {
      this.closeElement.on('click touchend', this._onCloseClickFn);
  ...
  lm.utils.copy(lm.controls.Tab.prototype, {
    ...
    /**
     * Sets this tab's isActive state. To programmatically
    ...
    setActive: function (isActive) {
      ...
      if (isActive) {
        ...
        if (this.contentItem && this.contentItem.parent)
          this.contentItem.parent.select()
      } else {
  ...
  lm.utils.copy(lm.items.Stack.prototype, {
    ...
    _$init: function () {
      ...
      if (this.contentItems.length > 0) {
        initialItem = this.contentItems[(this.config.activeItemIndex == null || this.contentItems.length >= this.config.activeItemIndex) ? 0 : this.config.activeItemIndex];
  ...
  lm.utils.copy(lm.utils.ConfigMinifier.prototype, {
    ...
    minifyConfig: function (config) {
      // Issue #110 (bad minifying of stockChartX state)
      // var min = {};
