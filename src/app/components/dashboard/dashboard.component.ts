...
import { NzConfigService, NzDropdownMenuComponent } from 'ng-zorro-antd';
...
export class DashboardComponent implements AfterViewInit, OnInit {
  ...
  ngOnInit() {
    ...
    // Sub menue not opened in correct place
    // It is happened after we disable mouse events in zone
    // Other solutions: modified library or create own menu component
    // If it happens in other components(places) thisnk about return some event in zone detection
    const setValue = NzDropdownMenuComponent.prototype.setValue
    NzDropdownMenuComponent.prototype.setValue = (...args) => {
      this._zone.run(() => setValue(...args))
    };
