...
export class LazyLoadingService {
  ...
  async loadGoldenLayout(): Promise<boolean> {
    return this.loadScript(scripts.GoldenLayout);
  }

  async loadScx(): Promise<boolean[]> {
    return Promise.all([
      this.loadStyles(styles.StockChartX),
      this.loadScripts(scripts.StockChartX)
    ]);
  }
